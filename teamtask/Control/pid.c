...
void Pid_parameter_init(void){
	Pid_increment_struct_init(&motor_pid_g[0].loc,  0.15, 	0.001,	0.035,   100,    -100);
	Pid_increment_struct_init(&motor_pid_g[0].spd,  24.0, 	1.2,	  0.03,  12000,  -12000);
	
	Pid_increment_struct_init(&motor_pid_g[1].loc,  0.364706f,	0.007333f,	0.004745f,   100,    -100);
	Pid_increment_struct_init(&motor_pid_g[1].spd,  16.7f,	0.98f,	0.024314f,  12000,  -12000);
	
	Pid_increment_struct_init(&(motor_pid_g[2].loc),  0.15, 	0.001,	0.035,   100,    -100);
...
//	Pid_increment_struct_init(&motor_pid_g[3].spd,  16.7f,	0.98f,	0.024314f,  12000,  -12000);
	...
	Pid_increment_struct_init(&motor_pid_g[4].loc,  0.15, 	0.001,	0.03,   100,    -100);
	Pid_increment_struct_init(&motor_pid_g[4].spd,  24.0, 	1.2,	  0.035,   9500,  -9500);
	
	Pid_increment_struct_init(&motor_pid_g[5].loc, 0.364706f,	0.007333f,	0.004745f,   100,    -100);
	Pid_increment_struct_init(&motor_pid_g[5].spd,   16.7f,	0.98f,	0.024314f,  12000,  -12000);
...
//	if(pid_struct==&motor_pid_g[1].loc && Basic_float_abs(position-target)<40){
